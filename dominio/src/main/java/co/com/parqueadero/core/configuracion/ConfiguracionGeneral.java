package co.com.parqueadero.core.configuracion;

public final class ConfiguracionGeneral {

    private ConfiguracionGeneral() {
    }

    public static final Integer VALOR_HORA_CARRO = 1000;
    public static final Integer VALOR_HORA_MOTO = 500;
    public static final Integer VALOR_DIA_CARRO = 8000;
    public static final Integer VALOR_DIA_MOTO = 4000;
    public static final Integer MAXIMO_CUPO_CARROS = 20;
    public static final Integer MAXIMO_CUPO_MOTOS = 10;

}
